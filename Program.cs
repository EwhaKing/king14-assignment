...
namespace TextRPG_1
{
    class Program
    {
        ...
        struct Monster
        {
            public MonsterType Type; // 몬스터의 종류를 나타내는 필드 추가
            public int hp;
            public int attack;
        ...
        Monster slime = new Monster(20, 2);
        Monster orc = new Monster(40, 5);
        Monster skeleton = new Monster(30, 7);
        ...
        static void CreateRandomMonster(out Monster monster)
        {
            ...
            int randomNumber = random.Next(1, 4); // 1 이상 4 미만의 랜덤 숫자 생성

            // 랜덤한 숫자에 따라 몬스터를 생성합니다.
            switch (randomNumber)
            {
                case 1:
                    InitializeMonster(out monster, MonsterType.Slime);
                    Console.WriteLine("슬라임이 생성되었습니다.");
                    break;
                case 2:
                    InitializeMonster(out monster, MonsterType.Orc);
                    Console.WriteLine("오크가 생성되었습니다.");
                    break;
                case 3:
                    InitializeMonster(out monster, MonsterType.Skeleton);
                    Console.WriteLine("스켈레톤이 생성되었습니다.");
                    break;
                default:
                    // 기본값인 경우, 체력과 공격력을 0으로 초기화합니다.
                    monster = new Monster();
                    break;
            }
            
        }
        static void InitializeMonster(out Monster monster, MonsterType type)
        {
            // 몬스터 타입에 따라 체력과 공격력을 초기화합니다.
            switch (type)
            {
                ...
                case MonsterType.Skeleton:
                    monster = new Monster() { Type = MonsterType.Skeleton, hp = 30, attack= 7 };
                    break;
                default:
                    // 기본값인 경우, 체력과 공격력을 0으로 초기화합니다.
                    monster = new Monster();
