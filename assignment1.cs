...
namespace TextRPG_1
{
    class Program
    {
        ...
        static ClassType ChooseClass()
        {
            ...
            ClassType choice = ClassType.None;  //ClassType 열거형 변수 choice에 none 할당
        ...
        static void CreateRandomMonster(out Monster monster)
        {
            ...
            int mNumber = random.Next(1, 3);
            
            MonsterType mChoice = MonsterType.None;
            
            switch (mNumber)
            {
                case 1:
                    mChoice = MonsterType.Slime;
                    monster.hp = 20;
                    monster.attack = 2;
                    Console.WriteLine("슬라임이 생성되었습니다.");
                    break;
                case 2:
                    mChoice = MonsterType.Orc;
                    monster.hp = 40;
                    monster.attack = 5;
                    Console.WriteLine("오크가 생성되었습니다.");
                    break;
                case 3:
                    mChoice = MonsterType.Skeleton;
                    monster.hp = 30;
                    monster.attack = 7;
        ...
        static void Fight(ref Player player, ref Monster monster)
        {
            while (true)
            {
                monster.hp -= player.attack; //플레이어가 먼저 몬스터 공격 
                if (monster.hp <= 0) 
                {
                ...
                player.hp -= monster.attack; //몬스터가 플레이어 공격
                if (player.hp <= 0)
                {
